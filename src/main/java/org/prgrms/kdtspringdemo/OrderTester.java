...
//        System.out.println(MessageFormat.format("{0}", orderProperties.getDescription()));
        ...
        logger.error(MessageFormat.format("Resource -> {0}", resource.getClass().getCanonicalName()));
        logger.warn(strings.stream().reduce("",(a,b)->a+"\n"+b));
        logger.warn(MessageFormat.format("is Jdbc Repo -> {0}", voucherRepository instanceof JdbcVoucherRepository));
